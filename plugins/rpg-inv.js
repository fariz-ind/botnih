...
let handler = async (m, { conn, usedPrefix }) => {
   ...
   // let name = m.fromMe ? conn.user : conn.contacts[m.sender]
    ...
    armor == 2 ? 'Iron Armor' :
    armor == 3 ? 'Gold Armor' :
    armor == 4 ? 'Diamond Armor' :
    armor == 5 ? 'Netherite Armor' :
    'Unknown Armor';


    let str = `*Inventory ${await conn.getName(name)}* 🎒
...
9. 🗑️ Top Sampah *${userssampah.indexOf(m.sender) + 1}* dari *${userssampah.length}*
`.trim()
